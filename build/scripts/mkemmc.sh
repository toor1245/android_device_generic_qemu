...
if [ "$bootsz" -gt 0 ]; then
    ...
    dd if="$bootimg1" of="$outimg" bs=128K count=$blocks \
        status=none
    check_truncation "$bootimg2" "$bootsz"
    dd if="$bootimg2" of="$outimg" bs=128K count=$blocks \
...
if [ "$rpmbsz" -gt 0 ]; then
    ...
    dd if="$rpmbimg" of="$outimg" bs=128K count=$blocks \
...
dd if="$userimg" of="$outimg" bs=128K seek=$(( pos / (128 * 1024) )) \
